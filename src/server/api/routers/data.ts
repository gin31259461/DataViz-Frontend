...
export const dataObjectRouter = createTRPCRouter({
  getDataObjectCount: publicProcedure
    .input(z.number())
  ...
  getSomeDataObject: publicProcedure
    .input(
      z.object({ order: z.string(), start: z.number(), counts: z.number() }),
    )
    .query(async ({ input, ctx }) => {
  ...
  getAllDataObject: publicProcedure
    .input(z.number())
    .query(async ({ input, ctx }) => {
  ...
  getLastObjectID: publicProcedure
    .input(z.number())
    .query(async ({ input, ctx }) => {
